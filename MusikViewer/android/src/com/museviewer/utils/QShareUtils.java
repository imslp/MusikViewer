...
public class QShareUtils
{
    ...
    private static String AUTHORITY="com.museviewer.sharex.fileprovider";

    protected QShareUtils()
    {
       //Log.d("MuseViewer", "QShareUtils()");
    }
    ...
    public static boolean checkMimeTypeView(String mimeType) {
        ...
        if (myIntent.resolveActivity(QtNative.activity().getPackageManager()) != null) {
            Log.d("MuseViewer checkMime ", "YEP - we can go on and View");
            return true;
        } else {
            Log.d("MuseViewer checkMime", "sorry - no App available to View");
        }
        return false;
    }
    ...
    public static boolean checkMimeTypeEdit(String mimeType) {
        ...
        // Verify that the intent will resolve to an activity
        if (myIntent.resolveActivity(QtNative.activity().getPackageManager()) != null) {
            Log.d("MuseViewer checkMime ", "YEP - we can go on and Edit");
            return true;
        } else {
            Log.d("MuseViewer checkMime", "sorry - no App available to Edit");
        }
        return false;
    }
    ...
    public static boolean share(String text, String url) {
        ...
        if (sendIntent.resolveActivity(QtNative.activity().getPackageManager()) != null) {
            ...
            Log.d("MuseViewer share", "Intent not resolved");
        }
        return false;
    }
    ...
    public static boolean createCustomChooserAndStartActivity(Intent theIntent, String title, int requestId, Uri uri) {
        ...
        if(defaultAppInfo == null) {
            Log.d("MuseViewer", title+" PackageManager cannot resolve Activity");
            return false;
        }
        ...
        // if (!defaultAppInfo.activityInfo.name.endsWith("ResolverActivity") && !defaultAppInfo.activityInfo.name.endsWith("EditActivity")) {
            // Log.d("MuseViewer", title+" defaultAppInfo not Resolver or EditActivity: "+defaultAppInfo.activityInfo.name);
            // return false;
        //}
        ...
        if (appInfoList.isEmpty()) {
            Log.d("MuseViewer", title+" appInfoList.isEmpty");
            return false;
        }
        Log.d("MuseViewer", title+" appInfoList: "+appInfoList.size());
        ...
        for (ResolveInfo appInfo : appInfoList) {
            ...
            if(isLowerOrEqualsKitKat) {
                Log.d("MuseViewer", "legacy support grantUriPermission");
        ...
        if (targetedIntents.isEmpty()) {
            Log.d("MuseViewer", title+" targetedIntents.isEmpty");
            return false;
        }
        ...
        if (targetedIntents.isEmpty()) {
            Log.d("MuseViewer", title+" only one Intent left for Chooser");
        } else {
        ...
        Log.d("MuseViewer", title+" Chooser Intent not resolved. Should never happen");
        return false;
    }
    ...
    public static boolean sendFile(String filePath, String title, String mimeType, int requestId) {
        ...
        try {
            ...
            Log.d("MuseViewer sendFile - cannot be shared: ", filePath);
            return false;
        }

        Log.d("MuseViewer sendFile", uri.toString());
        sendIntent.putExtra(Intent.EXTRA_STREAM, uri);

        if(mimeType == null || mimeType.isEmpty()) {
            // fallback if mimeType not set
            mimeType = QtNative.activity().getContentResolver().getType(uri);
            Log.d("MuseViewer sendFile guessed mimeType:", mimeType);
        }  else {
            Log.d("MuseViewer sendFile w mimeType:", mimeType);
        }
    ...
    public static boolean viewFile(String filePath, String title, String mimeType, int requestId) {
        ...
        try {
            ...
            Log.d("MuseViewer viewFile - cannot be shared: ", filePath);
            return false;
        }
        // now we got a content URI per ex
        // content://org.MuseViewer.examples.sharex.fileprovider/my_shared_files/qt-logo.png
        // from a fileUrl:
        // /data/user/0/org.MuseViewer.examples.sharex/files/share_example_x_files/qt-logo.png
        Log.d("MuseViewer viewFile from file path: ", filePath);
        Log.d("MuseViewer viewFile to content URI: ", uri.toString());

        if(mimeType == null || mimeType.isEmpty()) {
            // fallback if mimeType not set
            mimeType = QtNative.activity().getContentResolver().getType(uri);
            Log.d("MuseViewer viewFile guessed mimeType:", mimeType);
        } else {
            Log.d("MuseViewer viewFile w mimeType:", mimeType);
        }
    ...
    public static boolean editFile(String filePath, String title, String mimeType, int requestId) {
        ...
        try {
            ...
            Log.d("MuseViewer editFile - cannot be shared: ", filePath);
            return false;
        }
        Log.d("MuseViewer editFile", uri.toString());

        if(mimeType == null || mimeType.isEmpty()) {
            // fallback if mimeType not set
            mimeType = QtNative.activity().getContentResolver().getType(uri);
            Log.d("MuseViewer editFile guessed mimeType:", mimeType);
        } else {
            Log.d("MuseViewer editFile w mimeType:", mimeType);
        }
        ...
        try {
                ...
                if (iStream != null) {
                    ...
                    if (name != null) {
                        ...
                        Log.d("MuseViewer - create File", filePath);
                        File f = new File(filePath);
                        FileOutputStream tmp = new FileOutputStream(f);
                        Log.d("MuseViewer - create File", "new FileOutputStream");
