...
QString MuseScore::getSaveScoreName(const QString& title,
                                    const QString& name, const QString& filter, bool selectFolder)
{
    QString _name;
...
void MuseScore::exportFile()
{
    ...
    if (fi.suffix().isEmpty())
        qDebug() << tr("Cannot determine file type");
    else
...
bool MuseScore::saveAs(Score* cs, bool saveCopy, const QString& path, const QString& ext)
{
    ...
    if (!rv){
        ...
        qDebug() << QString("Cannot write into %1").arg(fn);
        emit errorSignal(QString("Cannot write into %1").arg(fn));
    }
    if (layoutMode != cs->layoutMode())
        cs->endCmd(true);       // rollback
    return rv;
...
bool MuseScore::saveAs(Score *cs, bool saveCopy, int fd, const QString &ext , int currpage)
{
    bool rv = false;
    QString suffix = "." + ext;
    //    QString fn(path);
    ...
    if (ext == "mscx" || ext == "mscz") {
        ...
        QFile f;
        if(!f.open(fd , QFile::ReadWrite)){
            qDebug() << "Unable to open file to write mscx/mscz files";
            return false;
        }

        try {
            if (ext == "mscz"){
                fi.setFile( QStandardPaths::writableLocation(QStandardPaths::TempLocation) + "/Temp.mscz");

                cs->saveCompressedFile(fi, false);
            }else
                fi.setFile( QStandardPaths::writableLocation(QStandardPaths::TempLocation) + "/Temp.mscx");
            cs->saveFile(fi);

            QFile f1(fi.absoluteFilePath());
            if(!f1.open(QFile::ReadOnly)){
                qDebug() << "Unable to Open f1 to read msc(x) file";
                emit saveCompleted();
                return false;
            }
            QFile f2;
            if(!f2.open(fd, QFile::ReadWrite)){
                qDebug() << "Unable to Open f2 to write msc(x) file";
                emit saveCompleted();
                return false;
            }

            QByteArray data = f1.readAll();
            f2.write(data);
            f2.close();
    ...
    //    }
        ...
        qDebug("Internal error: unsupported extension <%s>",qPrintable(ext));
        emit saveCompleted();
        return false;
    }
    if (!rv){
        //QMessageBox::critical(this, tr("MuseScore:"), tr("Cannot write into %1").arg(fn));
        qDebug() << QString("Cannot write into %1").arg(fd);
        emit errorSignal(QString("Cannot write into %1").arg(fd));
    }
    if (layoutMode != cs->layoutMode())
        cs->endCmd(true);       // rollback
    emit saveCompleted();
...
bool MuseScore::saveMidi(Score* score, int fd)
{
    ExportMidi em(score);
    return em.write(fd, preferences.midiExpandRepeats);
}
...
bool MuseScore::savePdf(const int fd)
{
    return savePdf(cs, fd);
}
...
bool MuseScore::savePdf(Score *score, int fd)
{
    score->setPrinting(true);
    MScore::pdfPrinting = true;
    QFile file;
    if(!file.open(fd , QFile::ReadWrite)){
        qDebug() << "Unable to open file to write pdf";
        return false;
    }
    QPdfWriter  printerDev(&file);
    printerDev.setResolution(preferences.exportPdfDpi);
    const PageFormat* pf = score->pageFormat();
    printerDev.setPageSize(QPageSize(pf->size(), QPageSize::Inch));
...
bool MuseScore::saveAs(Score* cs, bool saveCopy)
{
    ...
    if (fi.suffix().isEmpty()) {
        qDebug() << "Cannot determine file type";
        emit errorSignal("Cannot determine file type");
        return false;
    }
...
bool MuseScore::saveSelection(Score* cs)
{
    if (!cs->selection().isRange()) {
        qDebug() <<tr("Please select one or more measures");
        emit errorSignal(tr("Please select one or more measures"));
        return false;
    }
    QStringList fl;
    fl.append(tr("MuseScore File") + " (*.mscz)");
    ...
    if (ext.isEmpty()) {
        ...
        qDebug() <<tr("Cannot determine file type");
        emit errorSignal(tr("Cannot determine file type"));
        return false;
    }
    bool rv = true;
    try {
        ...
        qDebug() <<tr("Save Selected");
        emit errorSignal(tr("Save Selected") +" " + s);
    }
    return rv;
}
...
bool MuseScore::savePng(Score* score, int fd , int currpage)
{
    return savePng(score, fd, false, false, converterDpi, trimMargin, QImage::Format_ARGB32_Premultiplied , currpage);
}
...
bool MuseScore::savePng(Score* score, int fd, bool screenshot, bool transparent, double convDpi, int trimMargin, QImage::Format format , int _page)
{
    qDebug() << "Page to export" << _page << "Convdpi" << convDpi;
...
//    buffer.open(QIODevice::WriteOnly);
    ...
    QFile file;
    if(!file.open( fd , QFile::ReadWrite)){
        qDebug() << "Unable to Open file png";
        return false;
    }

    rv = printer.save(&file, "png");
...
//    file.write(ba);
    ...
    //        qDebug() << "Unable to Open file png1";
    //        return false;
    //    }

    //    QFile f1;
    //    if(!f1.open( fd ,QFile::WriteOnly)){
    //        qDebug() << "Unable to Open file png2";
    //        return false;
    //    }


    //    f1.write(file.readAll());
    //    f1.close();
    //    file.close();
