...
bool saveXml(Score* score, int fd)
      {
      QFile f;
...
bool saveMxl(Score* score, int fd)
      {
      MQZipWriter uz("temp");
      ...
      if (MScore::debugMode)
            ...
            dt = QDateTime::currentDateTime();
#endif
      QString fn = "temp.xml"; //fi.completeBaseName() + ".xml";
      ...
      if(!f1.open((QFile::ReadOnly))){
          qDebug() << "Unable to Open f1";
          return false;
      }
      QFile f2;
      if(!f2.open(fd , QFile::ReadWrite)){
          qDebug() << "Unable to Open f2";
          return false;
      }


      QByteArray alldata = f1.readAll();
      f2.write(alldata);
      f2.close();
      f1.close();
