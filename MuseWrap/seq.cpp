...
void Seq::setState(Transport _state)
{
    state = _state;
    if(state == Transport::PLAY){
       emit toGui('1');
    }else if(state == Transport::STOP){
        toGui('2');
    }
}
...
void Seq::process(unsigned framesPerPeriod, float* buffer)
{
    ...
    if (driverState != state) {
        ...
        if (state == Transport::STOP && driverState == Transport::PLAY) {
            if ((preferences.useJackMidi || preferences.useJackAudio) && !play) {
...
//     qDebug() << "Process took" << timer.elapsed() << "ms";
}
...
void Seq::threadStarted()
{
    ...
    if(noteTimer!=nullptr){
        noteTimer->deleteLater();
    }
