...
bool MuseScore::saveAudio(Score* score, QIODevice *device, std::function<bool(float)> updateProgress)
{
    ...
    for (int pass = 0; pass < 2; ++pass) {
        ...
        for (;;) {
            ...
            qDebug() << "Internal For Time " << timer2.elapsed();
            if (frames) {
        ...
        qDebug() << "External For time " << timer1.elapsed();
        if (cancelled)
            break;
        if (pass == 0 && peak == 0.0) {
...
bool MuseScore::saveAudio(Score* score, int fd , QString ext )
{
    // QIODevice - SoundFile wrapper class
    class SoundFileDevice : public QIODevice {
    private:
        ...
        int fd;
    public:
        SoundFileDevice(int sampleRate, int format, int fd)
            : fd(fd) {
            memset(&info, 0, sizeof(info));
            info.channels   = 2;
            info.samplerate = sampleRate;
        ...
        bool open(QIODevice::OpenMode mode) {
            ...
            sf     = sf_open_fd(fd, SFM_WRITE, &info , SF_TRUE);
            if (sf == 0) {
                qDebug("open soundfile failed: %s", sf_strerror(sf));
                return false;
            }
    ...
    SoundFileDevice device(sampleRate, format, fd);

    // dummy callback function that will be used if there is no gui
    std::function<bool(float)> progressCallback = [this](float progress) { emit exportProgress(progress) ;
        bool cancelled = m_exportCancelled;
